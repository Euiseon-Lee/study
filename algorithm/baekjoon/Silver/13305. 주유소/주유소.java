...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        int[] arrDistance = new int[cityCount];
        int[] arrOilPrice = new int[cityCount];
        int totalDistance = 0;
        for (int i = 0; i < cityCount; i++) {
            int tempDistance = Integer.parseInt(st1.nextToken());
            arrDistance[i] = tempDistance;
            totalDistance += tempDistance;
            arrOilPrice[i] = Integer.parseInt(st2.nextToken());
        }
        int idx = 0;
        int boughtDistance = arrDistance[0];
        int currentOilPrice = arrOilPrice[0];
        int boughtPrice = arrDistance[0] * currentOilPrice;
